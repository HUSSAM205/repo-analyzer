...
export function WorkspaceShell({
  ...
  // Seeded from `?tab=chat` (the sidebar's "AI Chatbot" link) so landing
  // here on a narrow/tabbed viewport opens straight into the chat pane
  // instead of the "files" default -- on a wide viewport all three panes
  // are visible regardless, so this only matters for the narrow case.
  const searchParams = useSearchParams();
  const [activeTab, setActiveTab] = useState<TabKey>(() =>
    searchParams.get("tab") === "chat" ? "chat" : "files"
  );
