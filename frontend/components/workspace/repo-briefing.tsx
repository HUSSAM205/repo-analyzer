...
export function RepoBriefing({ briefing }: { briefing: DomainBriefing | null | undefined }) {
  ...
  if (!briefing) {
    return (
      <div className="border-b border-zinc-800/60 px-4 py-3 text-xs text-zinc-500">
    ...
    <section className="glass border-b border-zinc-800/60 px-4 py-3">
      ...
      >
        <div className="flex min-w-0 items-center gap-2">
          ...
          {!expanded && (
            <span className="truncate text-xs text-zinc-500">{briefing.target_audience}</span>
          )}
        </div>
        {expanded ? (
        ...
        {expanded && (
          ...
          >
            <div className="space-y-3 pt-3">
              <p className="text-xs text-zinc-400">{briefing.target_audience}</p>
              ...
              <p className="text-sm leading-relaxed text-zinc-300">{briefing.architecture_overview}</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
