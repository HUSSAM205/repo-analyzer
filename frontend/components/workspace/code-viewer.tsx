...
// A category the backend never actually sends today (see
// backend/app/core/code_annotation.py -- category always comes from local
// AST segmentation, not the LLM, so it's always one of the four keys
// above) -- kept only as a render-time guard so a future backend change or
// an unexpected API response can never index CATEGORY_CONFIG with an
// undefined result and crash the tree, per the "no missing/changed field
// may ever crash render" rule.
const UNKNOWN_CATEGORY_CONFIG = {
  label: "Other",
  badgeClass: "border-zinc-700 bg-zinc-800/40 text-zinc-400 ring-zinc-700/30",
};
...
export function CodeViewer({ repoId, path }: { repoId: string; path: string | null }) {
  ...
  // Set only when the file content itself loaded fine but the syntax
  // highlighter threw (e.g. a CSP/WASM-init failure) -- kept distinct from
  // `error` so a highlighter-only failure never hides real, already-fetched
  // content behind a misleading "could not load" message.
  ...
  useEffect(() => {
    ...
    apiFetch(`/api/repos/${repoId}/files/content?path=${encodeURIComponent(path)}`, { cache: "no-store" })
      ...
      .then((data) => {
        ...
        // Highlighting is handled in its own try/catch, separate from the
        // fetch above: the content has already loaded successfully at this
        // point, so a highlighter-only failure (e.g. a CSP blocking Shiki's
        // WASM engine) must fall back to plain text, not the generic
        // "Could not load this file's content" error -- that message should
        // only ever mean the fetch itself failed.
        highlightCode(data.content, path)
          .then((highlighted) => {
  ...
  async function handleCopy() {
    // See readme-generator.tsx's handleCopy for why this is wrapped: a
    // denied clipboard permission rejects and must not become an unhandled
    // promise rejection.
    try {
  ...
  if (!tooLarge && html === null && !highlightFailed) {
    return (
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {mode === "raw" ? (
        highlightFailed ? (
          <pre className="flex-1 overflow-auto whitespace-pre p-4 font-mono text-sm leading-relaxed text-zinc-300">
...
function AnnotatedViewBody({
  ...
  return <AnnotatedBlockList annotations={annotations} blockHtml={blockHtml} />;
}

// Split out so its collapsed/expanded state is real component-local state
// (not re-derived every render) -- reset per file via the `key={path}` this
// is mounted under in CodeViewer, so switching files never carries over
// which cards were expanded for the previous one.
function AnnotatedBlockList({
  annotations,
  blockHtml,
}: {
  annotations: FileAnnotationsResponse;
  blockHtml: string[];
}) {
  // All expanded by default (matches the reading experience before this
  // became collapsible) -- collapsing is an opt-out for a block a reader
  // already understands, not the default state.
  ...
  return (
    <div className="flex-1 overflow-auto p-4">
      {annotations.blocks.map((block, i) => {
        const config = CATEGORY_CONFIG[block.category] ?? UNKNOWN_CATEGORY_CONFIG;
        const isExpanded = !collapsed.has(i);
        return (
          <motion.div
          ...
          >
            ...
            <div
              className="overflow-auto rounded-md border border-zinc-800/60 bg-card/40 p-3 font-mono text-sm leading-relaxed transition-colors duration-150 hover:border-primary/40 hover:bg-card/70 [&_pre]:!bg-transparent [&_pre]:whitespace-pre"
            ...
            <button
              ...
              className="glow-pill glass mt-2 flex w-full items-center justify-between gap-2 rounded-md px-3 py-1.5 text-left text-[11px] font-semibold uppercase tracking-wide text-primary/80 transition-colors hover:text-primary"
            >
              ELI5 Explanation
              <motion.span animate={{ rotate: isExpanded ? 180 : 0 }} transition={{ duration: 0.15, ease: "easeInOut" }}>
            ...
            <AnimatePresence initial={false}>
              {isExpanded && (
                ...
                >
                  <div className="glass mt-2 space-y-2.5 rounded-md p-3">
                    <AnnotationField label="What this does" emoji="📌" value={block.logic_summary} />
                    <AnnotationField label="Why it exists & how it connects" emoji="💡" value={block.flow} />
                    <AnnotationField label="Tips for beginners" emoji="🚀" value={block.tips} />
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
...
function AnnotationField({ label, emoji, value }: { label: string; emoji: string; value: string }) {
  return (
    <div>
