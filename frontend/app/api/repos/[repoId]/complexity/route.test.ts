...
describe("GET /api/repos/[repoId]/complexity", () => {
  ...
  it("returns 401 without a session token", async () => {
    const response = await GET(new NextRequest("http://localhost/api/repos/x/complexity"), {
      params: { repoId: "x" },
    });
  ...
  it("passes through the backend's status and body unchanged", async () => {
    ...
    const body = {
      ...
      hotspots: [{ file: "app.py", function: "f", line: 1, complexity: 5, maintainability: 70, line_count: 20 }],
      disclaimer: "...",
    };
