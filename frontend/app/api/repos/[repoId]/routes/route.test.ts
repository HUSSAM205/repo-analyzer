...
describe("GET /api/repos/[repoId]/routes", () => {
  afterEach(() => {
    store.clear();
  ...
  it("returns 401 without a session token", async () => {
    const response = await GET(new NextRequest("http://localhost/api/repos/x/routes"), {
      params: { repoId: "x" },
    });
  ...
  it("passes through the backend's status and body unchanged", async () => {
    ...
    const body = { routes: [], frameworks_detected: [], disclaimer: "..." };
