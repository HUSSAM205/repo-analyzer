...
describe("GET /api/repos/[repoId]/tech-debt", () => {
  afterEach(() => {
    store.clear();
  ...
  it("returns 401 without a session token", async () => {
    const response = await GET(new NextRequest("http://localhost/api/repos/x/tech-debt"), { params: { repoId: "x" } });
    expect(response.status).toBe(401);
  });

  it("passes through the backend's status and body unchanged", async () => {
    store.set("session_token", "test-token");
    const body = { estimated_debt_hours: 3.5, summary: "Some debt.", items: [] };
