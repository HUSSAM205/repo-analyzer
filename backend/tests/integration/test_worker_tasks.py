...
async def test_analyze_repo_task_progresses_through_all_four_stages_in_order(monkeypatch):
    ...
    async with async_session_maker() as db:
        ...
        repo = Repo(
            user_id=user.id, url=f"https://example.com/fake/stage-repo-{uuid.uuid4()}", name="fake-repo",
    ...
    def fake_walk_and_chunk(root_dir, max_files, skip_chunking=False):
...
@pytest.mark.asyncio
async def test_analyze_repo_task_sanitizes_unexpected_error_message(monkeypatch):
    # Confirmed live: a raw asyncpg/SQLAlchemy exception's str() can include
    # the entire generated SQL statement and every bound parameter -- for
    # the batch File insert, that's real file content from the repo being
    # analyzed. Storing that directly in job.error_message (which the
    # frontend shows verbatim) leaked it straight to the user. The stored
    # message must stay short and generic regardless of how verbose/
    # sensitive the underlying exception's message is; the full exception
    ...
    async with async_session_maker() as db:
        ...
        repo = Repo(
            user_id=user.id, url="/nonexistent/sanitize-error", name="sanitize-error-repo",
            status=RepoStatus.PENDING,
        )
        db.add(repo)
        await db.flush()
    ...
    def fake_clone_repo(*args, **kwargs):
        raise RuntimeError(
            "INSERT INTO files (content) VALUES ('SUPER_SECRET_FILE_CONTENTS_LEAK')"
        )
    ...
    async with async_session_maker() as db:
        refreshed_job = await db.get(Job, uuid.UUID(job_id))
        assert refreshed_job.status == JobStatus.FAILED
        assert refreshed_job.error_message == "Unexpected error: RuntimeError"
        assert "SUPER_SECRET_FILE_CONTENTS_LEAK" not in refreshed_job.error_message
        refreshed_repo = await db.get(Repo, repo_id)
        assert refreshed_repo.status == RepoStatus.FAILED
...
async def test_analyze_repo_task_marks_failed_on_cancellation(monkeypatch):
    ...
    async with async_session_maker() as db:
        ...
        repo = Repo(
            user_id=user.id, url=f"https://example.com/fake/cancelled-repo-{uuid.uuid4()}", name="cancelled-repo",
            status=RepoStatus.PENDING,
        )
        db.add(repo)
        await db.flush()
