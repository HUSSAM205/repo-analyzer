...
@pytest.mark.asyncio
async def test_read_file_with_query_jumps_to_the_matching_section_of_a_long_file(db_session):
    # The real motivating case: a value buried deep in a long file that a
    # plain from-the-top read at a tight budget never reaches. Deliberately
    # NOT a `[section]` header or def/class line -- those are what
    # extract_outline finds even with no query at all (covered separately
    # below), so using one here wouldn't actually isolate
    # truncate_around_match's own behavior, which this test is about.
    repo = await _make_repo(db_session)
    filler = "\n".join(f"key{i} = {i}" for i in range(1000))
    content = f"{filler}\npytest_min_version = \"7.4.0\"\n{filler}"
    db_session.add(File(repo_id=repo.id, path="pyproject.toml", content=content))
    await db_session.flush()

    top_read = await read_file(db_session, repo.id, "pyproject.toml", max_tokens=50)
    assert "pytest_min_version" not in top_read

    targeted_read = await read_file(db_session, repo.id, "pyproject.toml", query="pytest", max_tokens=50)
    assert "pytest_min_version" in targeted_read
    assert "7.4.0" in targeted_read


@pytest.mark.asyncio
async def test_read_file_with_no_query_falls_back_to_a_structural_outline(db_session):
    # When the caller doesn't pass query at all (the model's own choice,
    # not guaranteed even when the prompt asks for it -- see agent.py's
    # comment on this), a structural outline is still strictly more useful
    # than blindly keeping only the top of the file when one is available.
    repo = await _make_repo(db_session)
    filler = "\n".join(f"key{i} = {i}" for i in range(1000))
    ...
    result = await read_file(db_session, repo.id, "pyproject.toml", max_tokens=50)

    assert "[tool.pytest.ini_options]" in result
    assert "structural outline" in result
    # The outline is section headers, not the ordinary key = value lines
    # around it (see extract_outline's docstring for why those aren't
    # treated as structural) -- the actual values still require query.
    assert "testpaths" not in result
