...
@pytest.mark.asyncio
async def test_get_repo_by_id_includes_latest_job(monkeypatch):
    ...
    #
    # Stubs out enqueueing (see test_analyze_enqueue_failure_marks_job_failed
    # for the pattern) -- a real docker-compose worker shares this test
    # suite's own Postgres/Redis and will actually pick up the enqueued job,
    # try to clone this fake URL, and fail it in the background, racing the
    # "status == pending" assertion below at a timing that varies with
    # worker load. Confirmed live: this test failed under a full-suite run
    # with the job already FAILED by the time the first assertion ran.
...
async def test_get_repo_by_id_includes_domain_briefing_and_job_stage():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        body2 = resp2.json()
        # A repo's stored domain_briefing JSONB predating the beginner-guide
        # fields (see DomainBriefing's schema) still validates -- the
        # response fills them in with their defaults rather than omitting
        # them or erroring.
        assert body2["domain_briefing"] == {
            **briefing,
            "beginner_summary": "",
...
@pytest.mark.asyncio
async def test_analyze_ready_repo_returns_existing_repo_no_new_job(monkeypatch):
    # Stub out enqueueing (see test_analyze_enqueue_failure_marks_job_failed
    # for the pattern) -- a real docker-compose worker shares this test
    # suite's own Postgres/Redis and *will* actually pick up an enqueued
    # job, try to clone this fake URL, and fail it in the background. That
    # flips repo.status to FAILED out from under the test at a timing that
    # varies with worker load, non-deterministically sending the second
    # request down the (correct, separately-tested) FAILED-repo
    # re-analysis path instead of the dedup path this test exists to check
    # -- a real bug was chased here before realizing the dedup logic was
    # never the problem, only the test's exposure to a real worker was.
    from app.api.routes import repos as repos_module
...
@pytest.mark.asyncio
async def test_analyze_pending_repo_returns_existing_repo_no_new_job(monkeypatch):
    # See the identical comment in test_analyze_ready_repo_returns_existing_repo_no_new_job
    # -- this test is exposed to exactly the same real-worker race (an
    # actually-enqueued job for this fake URL gets picked up and failed by
    # the docker-compose worker before the second request runs), just via
    # PENDING instead of READY.
    from app.api.routes import repos as repos_module
...
async def _wait_for_fallback_tasks(repos_module) -> None:
    # The fallback runs via a fire-and-forget asyncio.create_task, not
    # awaited by the endpoint itself -- give it a chance to finish before
    # asserting on DB state. If it already finished before this runs, its
    # done-callback already removed it from the set, so there's nothing to
    # await and this is a no-op either way.
    for task in list(repos_module._fallback_tasks):
        await task


@pytest.mark.asyncio
async def test_analyze_falls_back_to_in_process_execution_when_enqueue_fails(monkeypatch):
    # When pool.enqueue_job raises (Redis/ARQ unavailable), the endpoint must
    # not just fail the request -- it should fall back to running the exact
    # same analyze_repo() coroutine directly in-process and still return 202,
    # so a Redis outage doesn't block new repo analysis at all.
    from app.api.routes import repos as repos_module
    ...
    ran_with_job_id: str | None = None

    async def fake_analyze_repo(ctx, job_id):
        nonlocal ran_with_job_id
        ran_with_job_id = job_id
        async with async_session_maker() as db:
            job = await db.get(Job, uuid.UUID(job_id))
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        token = await _register_and_login(client)
        url = f"https://github.com/octocat/fallback-{uuid.uuid4()}"
        resp = await client.post(
        ...
        await _wait_for_fallback_tasks(repos_module)

        assert ran_with_job_id == body["job_id"]
        async with async_session_maker() as db:
...
@pytest.mark.asyncio
async def test_analyze_falls_back_to_in_process_execution_when_get_arq_pool_fails(monkeypatch):
    # get_arq_pool() itself (not just pool.enqueue_job) can raise -- e.g. the
    # very first call in a process, when Redis is refusing new connections
    # (quota exceeded, outage). Same fallback contract as the enqueue_job
    # failure above.
    from app.api.routes import repos as repos_module

    async def failing_get_arq_pool():
        raise ConnectionError("redis unavailable")

    monkeypatch.setattr(repos_module, "get_arq_pool", failing_get_arq_pool)
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        token = await _register_and_login(client)
        url = f"https://github.com/octocat/pool-fail-{uuid.uuid4()}"
        resp = await client.post(
            "/api/v1/repos/analyze", json={"repo_url": url}, headers={"Authorization": f"Bearer {token}"}
        )
        assert resp.status_code == 202, resp.text
...
@pytest.mark.asyncio
async def test_analyze_marks_job_failed_when_even_the_in_process_fallback_cannot_start(monkeypatch):
    # Last-resort net: if ARQ is down AND scheduling the in-process fallback
    # itself somehow raises, the job/repo must still end up FAILED (not left
    # stuck PENDING forever) and the request must still surface a clean 503.
    from app.api.routes import repos as repos_module
    ...
    def failing_schedule(job_id):
        raise RuntimeError("event loop unavailable")

    monkeypatch.setattr(repos_module, "_schedule_in_process_analysis", failing_schedule)

    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        token = await _register_and_login(client)
        url = f"https://github.com/octocat/total-fail-{uuid.uuid4()}"
        resp = await client.post(
            "/api/v1/repos/analyze", json={"repo_url": url}, headers={"Authorization": f"Bearer {token}"}
        )
        ...
        async with async_session_maker() as db:
            ...
            assert job.error_message == "Failed to start analysis job"
