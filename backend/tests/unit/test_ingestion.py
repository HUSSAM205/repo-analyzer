...
def test_walk_and_chunk_skips_files_containing_a_null_byte(tmp_path):
    # A NUL byte is legal UTF-8 (codepoint U+0000), so it survives
    # path.read_text() -- but Postgres text/varchar columns can't store one
    # at all, regardless of encoding validity. Confirmed live: a real repo
    # (github/linguist's test-fixture corpus) contains files like this and
    # crashed the batch File insert with an unhandled asyncpg
    # CharacterNotInRepertoireError.
    repo_dir = tmp_path / "repo_with_null_byte"
    shutil.copytree(FIXTURE_DIR, repo_dir)
    (repo_dir / "has_null.txt").write_bytes(b"before\x00after")

    result = walk_and_chunk(repo_dir, max_files=100)

    assert result.files_processed == 3  # only the 3 legitimate fixture files
    assert result.files_skipped == 1
    assert not any(f.path == "has_null.txt" for f in result.files)
    assert not any(c.file_path == "has_null.txt" for c in result.chunks)
