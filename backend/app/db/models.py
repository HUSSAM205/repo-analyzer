...
class Repo(Base):
    ...
    # Flagship-tools cache, all lazily generated on first request and cached
    # here from then on -- same "generate once, cache unless it was a pure
    # fallback" pattern as File.annotations (see app/api/routes/files.py).
    # None until a user has actually opened that tool for this repo.
    readme_doc: Mapped[str | None] = mapped_column(Text, nullable=True)
    security_scan: Mapped[list | None] = mapped_column(JSONB, nullable=True)
    health_score: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    quiz: Mapped[list | None] = mapped_column(JSONB, nullable=True)
    # Mermaid flowchart syntax (raw text) describing the repo's inferred
    # request-handling architecture -- see app.core.flow_map.
    flow_map: Mapped[str | None] = mapped_column(Text, nullable=True)
    # Technical-debt estimate + before/after refactor recipes -- see
    # app.core.tech_debt.
    tech_debt: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    # License-risk + secret-leak scan -- see app.core.compliance_scanner.
    # Unlike the other flagship-tool caches, this one is deterministic (no
    # LLM call) and therefore never has a "failed generation" case -- once
    # set, it's always a real result, never a retry-later placeholder.
    compliance_scan: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
...
class Conversation(Base):
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=_utcnow, nullable=False)
    # Rolling summary of every message older than the most recent
    # ROLLING_WINDOW_MESSAGES (see app/core/conversation_summary.py) --
    # what keeps a very long conversation's chat quality from degrading
    # once its full history no longer fits in the per-turn context window,
    # without re-summarizing the whole conversation from scratch on every
    # turn (see summary_covers_through_message_count below). None until the
    # conversation has grown past the rolling window at least once.
    summary: Mapped[str | None] = mapped_column(Text, nullable=True)
    # How many of this conversation's oldest messages are already folded
    # into `summary` -- lets each turn's background summarization step
    # (chat.py's send_message) fold in only the newly-aged-out slice since
    # the last update, an O(new messages) cost instead of O(total history)
    # regardless of how long the conversation has grown.
    summary_covers_through_message_count: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
